As continuation to the SQL counterpart, this R based project includes not only data wrangling using `dplyr`, `stringr` and `purrr` but also the visualization of the results using `ggplot2`. The attached data <a href="https://www.kaggle.com/datasets/dillonmyrick/bike-store-sample-database/data" >(click me)</a> was used to explore: 
...
Load needed libraries to wrangle and visualize the data effectively. 
```R
#load libraries
...
Load provided .csv files. Because the values corresponding to date columns were stored as characters, we need to adjust the shipped_date values using the `as.Date()`function to get rid of former NULL values by converting these to NAs. 
```R
#set working directory and load necessary input files
setwd('NA') #set your own directory
...
When the sales per quartile are of need, every quartile of every year can be subsetted, wrangled, sorted and output using following code:
```R
...
q1_2016
```
For a clear overview I'll leave out the code for the remaining quartile in the readme, but the code can be found, of course, in the attached .txt and .r file provided. 

Adjust the output dataframes to get a distinct summary of wanted parameters for visualization purposes.
```R
...
distinct_df_all
```
Visualize the resulting dataframe `distinct_df_all` using `ggplot2`. The plot becomes interactive using a simple function: `ggplotly()`. The interactive plot can surely be modified more, but for the only purpose to browse a bit, it's sufficient. 
```R
##############################################
...
The total profits are interesting, but how about the profits missed by giving discounts to the customers? Let's visualize that... Both plots can be combined using the `patchwork` package.
```R
#############################################################
...
combined
```
The profits look great, the given discounts cost money but eventually might have convinced the customer to buy the product. But what products perfomed best per year? To keep it simple, I reduced the questions to the top 3 best sellers. 
```R
#############################################################
...
#sort it to get identical colors across years
...
```
Let' visualize the differences and check out which products stood out the most! With this out of the way, we can combine all plots together in one for a good overview what the numbers of the previous quartiles and years looked like.
```R                                   
###2016###
...
Now, each product was sold by an employee. And each employee should get a bonus of 1% of the profit they made for the company.
```R
...
```
Does not sound like much, but let's prepare the data to also visualize that...
```R
...
ggplotly(bonus)
```
Well, two employees especially did very well and got also a great anual bonus of up to ~ $ 13 000 in 2017.
